# Source: /mydata/cerebra_fitness_zeroshot/openfold/get_all_atoms_new.py
# Local adaptation: package imports and resource location only.
import math
...
from .shape_constants import NUM_RES, NUM_EXTRA_SEQ, NUM_TEMPLATES, NUM_MSA_SEQ
from . import residue_constants as rc
from .rigid_utils import Rotation, Rigid
from .tensor_utils import (
    tree_map,
...
def hu_model_single_protein_extra_feat_get(seq,raw_all_atoms_pos,raw_all_atoms_mask,AncherList_single,seq_mask =None):
    # raw_all_atoms [L,37,3]
    nres = raw_all_atoms_pos.shape[0]
    device = raw_all_atoms_pos.device
    N_CA_C = raw_all_atoms_pos[:, :3, :]
    rotation = NormVec(N_CA_C)
    U, _, V = torch.svd(torch.eye(3).unsqueeze(0).permute(0, 2, 1).to(device) @ rotation)
    d = torch.sign(torch.det(U @ V.permute(0, 2, 1)))
    Id = torch.eye(3).repeat(nres, 1, 1).to(device)
    Id[:, 2, 2] = d
    ...
    of_seq = of_seq[None,...].repeat(num_k,1).to(device)
